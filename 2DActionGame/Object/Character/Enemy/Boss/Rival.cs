...
{
	/// <summary>
	///  Stage3のボス：主人公のライバルのクラス。似たような技を持たせる.メインは剣での攻撃
	///  12/5:主人公と同じ攻撃パターン&移動パターン+1,2の特殊技の実装で決定.
	///  なるべくPlayerのコードを再利用することにした。→本当は剣撃スキル的なクラスを作って、それをPlayerとRivalに持たせるべきなのかも
	/// </summary>
	public class Rival : Boss
	{
		...
		/// Hard用のパターン
		/// </summary>
		private int[] attackPattern1 = { 7, 1, 9 };
		#endregion
		#region Update
		/// <summary>
		...
		protected void InterceptA(float chance)
		{
			if (isWaiting && Vector2.Distance(position, stage.player.position) < 80)
				if (interceptTime % 60 == 0) triangle = true;
				else triangle = false;
		...
		private void AttackeWithWind(int type)
		{
			switch (type) {
				case 0:
		...
		private void AttackWithShuriken(int shootTime)
		{
			if (isStartingAttack) {
			...
			: base(stage, x, y, width, height, HP, bulletType, shootType)
		{
			...
			//weapons.Add(cutterTurret);
			//weapons.Add(syuriken);
		...
		protected override void Load()
		{
			...
			attackMethods.Add(attackMethodType2 = AttackWithShuriken);	// 8
		...
		public override void Draw(SpriteBatch spriteBatch)
		{
			if (isAlive && isActive) {
				if (!turnsRight) {
					...
					spriteBatch.Draw(texture, drawPos, animation.rect, Color.White, 0, Vector2.Zero, Vector2.One, SpriteEffects.FlipHorizontally, 0f);
				}
