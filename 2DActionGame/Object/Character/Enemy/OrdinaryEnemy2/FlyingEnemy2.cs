...
{
	/// <summary>
	/// Stage2 左右に揺れ動きながら氷弾をPayerに向けて撃つ敵
	/// </summary>
    public class FlyingEnemy2 : FlyingEnemy
    {
            ...
            : base(stage, x, y, width, height, HP, motionType)
        {
            ...
            turret = new Turret(stage, this, new Vector2(), 8, 8, 0, 5, 2, false, true, false, 1, 0, 3);
			stage.weapons.Add(turret);
            //turret.isBeingUsed = true;  // 忘れてた.このフラグは重要
        ...
        {// なぜかisRunnningSlowly=trueになるほど重くなる
            //MovementUpdate(210,45);
            if(isAlive) {
                if (!thread.isDivided) {
                ...
                turret.Update();    // 呼ばれてるがbulletが全然描画されない...
                base.Update();
            }
            else turret.isAlive = false;
        }
        ...
        protected override void UpdateNumbers()
        {
            ...
            if (speed.X > 0) {
				speed.X += (float)-(.40 * friction) * timeCoef;
